...
# ------------------------------------------------------------
# Configuro un logger quez envía todo a consola y a un archivo
# ------------------------------------------------------------
...
class PortfolioOptimizer:
    ...
    def optimize_portfolio(self, tickers, prices, year, quarter, initial_weights=None, capital=10000):
        """Optimiza el portafolio, calcula Sharpe ex-post y retorno CAPM ex-ante."""
        
        # 1. Validación de dimensiones
        if len(tickers) != prices.shape[1]:
            raise ValueError(f"Error crítico: {len(tickers)} tickers pero {prices.shape[1]} columnas de precios.")
        ...
        try:
            ...
            if returns.shape[1] != len(tickers):
                raise ValueError(f"Discrepancia: {returns.shape[1]} series vs {len(tickers)} tickers")

            # 5. Matriz de covarianzas y vector CAPM ex-ante
            cov = returns.cov().values
            exp_ret_arr = self.calculate_expected_returns(tickers, year, quarter)

            # 6. Extraer fechas de compra/venta
            end_dt = self.get_quarter_end_date(year, quarter)
            q_map_start = {'Q1': (1, 1), 'Q2': (4, 1), 'Q3': (7, 1), 'Q4': (10, 1)}
            m_s, d_s = q_map_start[quarter]
            quarter_start_dt = datetime(year, m_s, d_s)
            ...
            # 7. Tasa libre de riesgo diaria (exacta)
            Rf_trimestral = self.market_params['Rf_VEN']
            num_business_days = len(returns)
            Rf_daily = Rf_trimestral / num_business_days

            # 8. Función objetivo: Sharpe ex-ante (CAPM)
            def neg_sharpe_expected(w):
                port_return_exp = np.dot(w, exp_ret_arr)
                port_vol = np.sqrt(w @ cov @ w)
                return -(port_return_exp - Rf_trimestral) / (port_vol + 1e-10)

            # 9. Pesos iniciales y restricciones
            ...
            if not res.success:
                print(f"⚠️ Optimización subóptima: {res.message}. Usando pesos iniciales.")
                w = w0
            else:
                w = res.x

            # 11. β cartera y retorno CAPM ex-ante
            ...
            # 12. Cantidades e inversión (igual que antes)
            getcontext().prec = 10
            ...
            if total_gastado > Decimal(str(capital)):
                ...
                qty = [(q * adjustment).quantize(Decimal('0.0001')) for q in qty]
                invested = sum(q * p for q, p in zip(qty, init_prices_dec))
                commission = (invested * Decimal(str(tc))).quantize(Decimal('0.01'))
                total_gastado = invested + commission

            # 13. Valor final y ROI (igual que antes)
            final_val = sum(float(q) * float(p) for q, p in zip(qty, final_prices_sim))
            roi = (final_val - float(total_gastado)) / capital

            # 14. Calcular Sharpe ex-post
            ...
            # 15. Calcular CAPM ex-post (realizado):
            beta_p        = sum(w[i] * self.current_betas[t] for i, t in enumerate(tickers))
            Rm_real       = self.get_quarterly_rm(year, quarter)   # viene del CSV
            capm_expost   = Rf_trimestral + beta_p * (Rm_real - Rf_trimestral)

            # 16. Retornar todos los campos (incluyendo realized_sharpe y alias sharpe)
            return {
                'weights': w,
            ...
            return {
                ...
                'realized_sharpe': 0,   # ⬅ siempre incluimos esta clave
                'sharpe': 0,            # ⬅ alias también aquí
                'capm_expost': 0,
                'risk': 0,
                'overall_deviation': 0,
                'roi': 0,
    ...
    def print_results(self, results):
        ...
        # ----- definimos “alpha” aquí:
        alpha = roi_pct - rm_pct                        # alfa = ROI real – Retorno IBC

        vol_pct = opt['risk'] * 100                    # Volatilidad diaria en %
        overall_dev = opt['overall_deviation'] * 100   # Desv. anualizada en %
        sharpe_expost = opt['realized_sharpe']                           # Sharpe ratio
        capm_expost = results['capm_expost'] *100

        # Cálculo CAPM ex-post -----------------------------------
        beta_portfolio = results['beta']              # β de cartera
        capm_expost = rf + beta_portfolio * (rm - rf)  # fórmula ex-post
        capm_expost = results['capm_expost']
        capm_expost_pct = capm_expost * 100            # en %
        # ============================================================
