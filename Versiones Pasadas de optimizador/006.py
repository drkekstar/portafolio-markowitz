...
class PortfolioOptimizer:
    def __init__(self, data_folder='data_acciones', risk_premium_type='Country'):
        ...
        #Iniciar Filtered_dates
        self.filtered_dates = {ticker: [] for ticker in self.priority_tickers}
    ...
    def calculate_daily_betas(self, tickers, window=252):
        """
        Calcula la β diaria de cada ticker usando una ventana de 'window' días hábiles.
        β_i(t) = Cov(r_i_window, r_m_window) / Var(r_m_window),
        donde antes filtramos cualquier retorno diario > |50%| (lo marcamos NaN y rellenamos).
        Además guardamos, para cada ticker, las fechas que se filtraron por tener |retorno| > 50%.
        """
        # 1) Retornos diarios del IBC
        df_ibc = self.daily_ibc[['ret_m']].dropna()

        # 2) Para cada ticker, cargar precios diarios y calcular retorno diario
        dict_retornos = {}
        for t in tickers:
            path = os.path.join(self.data_folder, f"{t}.csv")
            df_t = pd.read_csv(path, parse_dates=['time'])
            df_t = df_t.set_index('time').sort_index()
            df_t = df_t[['close']].rename(columns={'close': f'precio_{t}'})
            df_t[f'ret_{t}'] = df_t[f'precio_{t}'].pct_change()
            dict_retornos[t] = df_t[[f'ret_{t}']]

        # 3) Concatenar retornos (alineados por fecha)
        all_ret = pd.concat([df_ibc] + list(dict_retornos.values()), axis=1).dropna()
        fechas = all_ret.index

        # 4) Preparar DataFrame donde iremos dejando las β
        df_betas = pd.DataFrame(index=fechas, columns=tickers, dtype=float)

        # 5) Función auxiliar para calcular β en una ventana dada
        def beta_ventana(r_i_win, r_m_win):
            cov = r_i_win.cov(r_m_win)
            var = r_m_win.var()
            return cov/var if var != 0 else np.nan

        # 6) Iterar fecha por fecha, desde window-1 en adelante
        for idx in range(window - 1, len(fechas)):
            ventana = all_ret.iloc[idx - window + 1: idx + 1].copy()
            r_m_win = ventana['ret_m']

            # 6.1) Filtrar retornos diarios extremos del IBC también (opcional)
            #      Si prefieres, puedes dejar el IBC sin filtrar; aquí solo muestro cómo:
            mask_m = ventana['ret_m'].abs() > 0.5
            if mask_m.any():
                # Rellenar con NaN y luego ffill/bfill
                ventana.loc[mask_m, 'ret_m'] = np.nan
                ventana['ret_m'] = ventana['ret_m'].ffill().bfill()

            # 6.2) Para cada ticker, filtrar sus propios retornos >|50%|
            for t in tickers:
                col_ret = f'ret_{t}'
                # Identificar fechas donde |retorno| > 0.5
                mask_i = ventana[col_ret].abs() > 0.5
                if mask_i.any():
                    # Guardar esas fechas en self.filtered_dates[t]
                    fechas_extremos = ventana.index[mask_i].tolist()
                    # Solo añadimos nuevas (evitar duplicados)
                ...
                # Rellenar NaN del ticker con ffill/bfill
                ventana[col_ret] = ventana[col_ret].ffill().bfill()

            # 6.3) Ahora que ya eliminamos outliers y rellenamos, calculamos β para cada ticker
            for t in tickers:
                r_i_win = ventana[f'ret_{t}']
                df_betas.at[fechas[idx], t] = beta_ventana(r_i_win, ventana['ret_m'])

        # 7) Finalmente, rellenar posibles NaN iniciales/finales
    ...
    def print_results(self, results):
        ...
        if results.get('transactions'):
            ...
            logger.info(tabulate(trans_table,
                                ...
                                tablefmt='grid'))
        #  X) Mostrar las fechas filtradas para cada ticker
        logger.info("FECHAS FILTRADAS (|retorno| > 50%) por acción:")
        for t in results['acciones']:
            fechas_t = self.filtered_dates.get(t, [])
            if fechas_t:
                # Formatear fechas como 'YYYY-MM-DD'
                lista_str = ", ".join(f.date().isoformat() for f in sorted(fechas_t))
                logger.info(f"  • {t}: {lista_str}")
            else:
                logger.info(f"  • {t}:  (ninguna)") 
        logger.info("")  # línea en blanco final
        # ——————————————————————————————————————————————
        logger.info("="*80 + "\n")
